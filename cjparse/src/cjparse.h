...
bool file_check(const char *file, FILE *return_file);
...
inline bool file_check(const char *file, FILE *return_file)
{
...
inline int file_error(FILE *fp, int error_code)
{
	errno = error_code;
	if (fp) fclose(fp);
	return true;
...
inline void print_count(int count, char *filename)
{
	printf("%d: %s\n", count, filename);
}
